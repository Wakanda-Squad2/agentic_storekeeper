...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-4 overflow-hidden rounded-2xl border border-[rgba(108,59,255,0.15)] bg-card py-4 text-sm text-card-foreground shadow-[0_0_20px_rgba(108,59,255,0.08)] backdrop-blur-sm animate-[neo-fade-in_0.45s_ease-out_forwards] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "font-heading text-base leading-snug font-semibold tracking-[0.5px] group-data-[size=sm]/card:text-sm",
        className
      )}
      {...props}
    />
